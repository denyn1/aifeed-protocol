...
function buildMcp() {
  for (const [source, target] of ENTRIES) {
    ...
    const targetPath = path.join(MCP_DIR, target);
...
if (require.main === module) {
  if (process.argv.includes('--check')) {
    const ok = checkMcp();
    process.stdout.write('mcp check: ' + (ok ? 'in sync' : 'OUT OF SYNC') + '\n');
    process.exitCode = ok ? 0 : 1;
  } else {
    const dir = buildMcp();
    process.stdout.write('mcp built: ' + path.relative(ROOT, dir) + '\n');
  }
}

module.exports = { buildMcp, checkMcp, MCP_DIR };
