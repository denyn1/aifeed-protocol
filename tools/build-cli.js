...
const CLI_DIR = path.join(ROOT, 'packages', 'aifeed-cli');
const ENTRIES = [
  ['bin', 'bin'],
  ['lib', 'lib'],
  ['schema', 'schema'],
...
function buildCli() {
  for (const [source, target] of ENTRIES) {
    ...
    const targetPath = path.join(CLI_DIR, target);
...
if (require.main === module) {
  if (process.argv.includes('--check')) {
    const ok = checkCli();
    process.stdout.write('cli check: ' + (ok ? 'in sync' : 'OUT OF SYNC') + '\n');
    process.exitCode = ok ? 0 : 1;
  } else {
    const dir = buildCli();
    process.stdout.write('cli built: ' + path.relative(ROOT, dir) + '\n');
  }
}

module.exports = { buildCli, checkCli, CLI_DIR };
