...
\begin{enumerate}
  ...
  asset, and triage metadata (including optional \code{mime}, \code{size}, and
  \code{sha-256} for verifiable asset downloads); the \mako{} compatibility profile;
  restrict-only per-page overrides; and a digest-bearing delta index with a site resume
  and per-entry triage fields.
...
links (\code{aifeed.assets}) so agents decide what to fetch; entries may carry
\code{mime}, \code{size}, and \code{sha-256} (hashed locally at build time), and clients
verify downloaded bytes against them before use (\code{verifyAsset}). The safe YAML
subset used for frontmatter rejects anchors, aliases, tags, flow collections, and
oversized scalars.
...
resume, and triage fields (title, summary, tags, language, related, asset count). Clients diff digests
...
parser abuses (anchors, aliases, tags, duplicate keys, depth/size bombs). Asset downloads
are bound to the page only by reference: when publishers declare \code{size} or
\code{sha-256}, clients verify the bytes before use; without them, asset integrity relies
on TLS alone. Two properties
...
(\code{@aifeed/verify}) with manifest, document, index, triage-selection, and asset
verification APIs; an
...
and oversized inputs. Asset integrity is exercised end-to-end: local builds hash declared
files, and downloads are verified byte-for-byte against \code{size}/\code{sha-256} in
unit and live checks. The WordPress end-to-end test verifies negotiation, inline
